...
.. hint::
  ...
  a frequency point. But instead of looping over each element explicitly,
...
For a symmetric and reciprocal structure (such as a passive two-conductor
transmission line), :math:`S_{12} = S_{21}` and :math:`S_{11} = S_{22}`.
When this applies, the full matrix can be filled from a single excitation::

    # only valid for passive, linear, reciprocal structures
    s22_list = s11_list
    s12_list = s21_list
...
To visualize the S-parameters, use `matplotlib`::
...
Deep notches in the return loss plot indicate frequencies where the DUT
delivers power into the load with minimal reflection, often corresponding
to resonance. A high return loss across a band suggests impedance mismatch
or a physical discontinuity in the structure.
...
Unexpectedly high insertion loss or anomalous frequency dependence may
indicate a modeling mistake, an impedance mismatch, or a physical effect
not accounted for in the design. These results are worth investigating by
reviewing the mesh, port placement, and boundary conditions.

The phase angle of :math:`S_{21}` wraps between −180° and 180°, which is
...
.. note::
   **Loss or Gain?**
   ...
   For more information, see *IEEE Std 1597.1-2008* and Elmore et al.,
   *Return Loss Sign Convention*, IEEE Antennas and Propagation Magazine, 2009.
...
.. important::
   We repeat the warning that was already given in the :ref:`concept_ports`
   ...
   Furthermore, even after de-embedding, it's not meaningful to associate this DUT's
   impedance with a pure capacitance, since this DUT is not a lumped capacitor but
   a parallel-plate waveguide. Only the RLCG transmission line is the correct
   model here.

Refactoring Post-Processing Code
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The matplotlib examples above repeat the same structure. In practice,
wrapping the plotting steps in a helper function keeps the code concise::
