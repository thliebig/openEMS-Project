...
#. **Perfectly Matched Layer (PML)**. This is the
   ...
   highest computational overhead of the available boundary conditions.
   Avoid it if efficiency is critical (e.g. only use PML at the simulation
   box's face directly hit by radiation, and use MUR for other boundaries).
   Intrusion of fringe fields and evanescent waves into the PML can
   destabilize it, causing the simulation to "blow-up". As a rule of thumb,
   radiating structures should be placed at least :math:`\lambda / 4` away
   from the PML boundary.
