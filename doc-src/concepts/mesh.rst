...
In addition to the algorithmic requirements, a mesh should also satisfy the
...
properly. Even when the mesh line and metal edge are aligned exactly,
...
.. note::
   If Port-based excitations are used (for simple use cases), openEMS
   automatically handles this low-level technical detail by correctly placing
