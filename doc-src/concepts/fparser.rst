...
3. Vary an excitation field's numerical value in space using a weighting function,
   ...
   waveguide, which must be excited appropriately to launch the desired propagation
   mode.  Transmission lines such as coaxial cables also work best with a custom
   field pattern to minimize artifacts due to an abrupt field input.
...
:func:`SetExcitationWeight`. Spatial variables cannot be used in
