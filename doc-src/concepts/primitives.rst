...
can create simple objects such as a ``Curve``, a ``Polygon``, a ``Box``, or a
...
A *primitive* is a geometrical shape that is always assigned to a
physical or non-physical material known as a *property*.
...
The Box is the simplest primitive in CSXCAD. It is also the most used primitive
...
surface using vertices and faces.
...
.. tabs::
   ...
   .. tab:: Octave
      ...
      * ``{'Matrix', [4x4 matrix]}``:
        ...
        * General affine transformation given as a 4 by 4 matrix. A 1 is
          appended internally to the row vector of the primitive nodes, so it
   ...
   .. tab:: Python
      ...
      * ``*args, **kw``: Argument of the transformation.
      ...
      All transformations applied to an object are internally tracked by the
